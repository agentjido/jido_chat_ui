defmodule JidoChatUIWeb.Layouts do
  ...
  def app(assigns) do
    ...
    <header class="navbar border-b border-base-300 px-4 sm:px-6 lg:px-8">
      <div class="flex-1">
        <.link navigate={~p"/rooms"} class="flex w-fit items-center gap-2 text-sm font-semibold">
          <span class="grid size-8 place-items-center rounded bg-base-300">JC</span>
          <span>Jido Chat UI</span>
        </.link>
      </div>
      <div class="flex-none">
        <ul class="flex flex-column items-center gap-1 px-1">
