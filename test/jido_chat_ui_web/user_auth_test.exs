defmodule JidoChatUIWeb.UserAuthTest do
  use JidoChatUIWeb.ConnCase, async: true

  alias JidoChatUI.Accounts
  alias JidoChatUI.Accounts.Scope
  alias JidoChatUIWeb.UserAuth

  import JidoChatUI.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, JidoChatUIWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
